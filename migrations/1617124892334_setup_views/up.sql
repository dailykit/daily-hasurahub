...
SELECT brand_customer.id,
   ...
   (
      SELECT subscription."customerSubscriptionReport"(
            brand_customer.id,
            'All'::text
...
SELECT "bulkItem"."supplierItemId",
   ...
   (
      ...
      WHERE ("supplierItem".id = "bulkItem"."supplierItemId")
   ) AS "supplierItemName",
   (
...
SELECT "sachetItem".id,
   ...
   (
      ...
      WHERE ("bulkItemView".id = "sachetItem"."bulkItemId")
   ) AS "supplierItemName",
   (
   ...
   (
      ...
      FROM inventory."bulkItem"
      WHERE ("bulkItem".id = "sachetItem"."bulkItemId")
   ) AS "bulkDensity"
   ...
   btrim(
      concat(
         (
            ...
            WHERE (product.id = "productOption"."productId")
         ),
         ' - ',
         "productOption".label
      )
...
CREATE VIEW "order"."cartItemView" AS WITH RECURSIVE parent AS (
   SELECT "cartItem".id,
      ...
      (
         ...
         WHERE ("cartItem".id = "cartItem_1"."parentCartItemId")
      ) AS count,
      CASE
   ...
   SELECT c.id,
      ...
      (
         ...
         FROM "order"."cartItem"
         WHERE ("cartItem"."parentCartItemId" = c.id)
      ) AS count,
      CASE
...
SELECT parent.id,
   ...
   COALESCE(
      ...
      (
         ...
         FROM products."productOption"
         WHERE ("productOption".id = parent."productOptionId")
      ),
      NULL::integer
...
SELECT (
   ...
   (
      SELECT subscription."assignWeekNumberToSubscriptionOccurence"("subscriptionOccurence".id) AS "subscriptionWeekRank"
      ...
      (
         ...
         WHERE (
               "subscriptionItemCount".id = (
                  SELECT "subscriptionOccurence"."subscriptionItemCountId"
                  FROM subscription."subscriptionOccurence"
                  WHERE (
...
CREATE OR REPLACE VIEW datahub_schema."columns" AS
SELECT columns.table_catalog,
   ...
   (
      SELECT concat('"', columns.table_schema, '"') AS concat
   ) AS schema_reference
FROM information_schema.columns
WHERE columns.table_schema::name <> ALL (
      ARRAY ['pg_catalog'::name, 'information_schema'::name]
   );
-- datahub_schema.columns_privileges source
CREATE OR REPLACE VIEW datahub_schema.columns_privileges AS
SELECT columns.table_catalog,
   columns.table_schema,
   columns.table_name,
   columns.column_name,
...
CREATE OR REPLACE VIEW datahub_schema.constraint_column_usage AS
SELECT columns.table_catalog,
   columns.table_schema,
   columns.table_name,
   columns.column_name,
...
CREATE OR REPLACE VIEW datahub_schema.constraint_table_usage AS
SELECT columns.table_catalog,
   columns.table_schema,
   columns.table_name,
   columns.column_name,
...
CREATE OR REPLACE VIEW datahub_schema.event_invocation_logs AS
...
CREATE OR REPLACE VIEW datahub_schema.event_log AS
SELECT event_log.id,
...
CREATE OR REPLACE VIEW datahub_schema.event_triggers AS
...
CREATE OR REPLACE VIEW datahub_schema.hdb_action AS
...
CREATE OR REPLACE VIEW datahub_schema.hdb_action_log AS
...
CREATE OR REPLACE VIEW datahub_schema.hdb_action_permission AS
...
CREATE OR REPLACE VIEW datahub_schema.hdb_computed_field AS
...
CREATE OR REPLACE VIEW datahub_schema.hdb_cron_event_invocation_logs AS
...
CREATE OR REPLACE VIEW datahub_schema.hdb_cron_events AS
...
CREATE OR REPLACE VIEW datahub_schema.hdb_cron_triggers AS
...
CREATE OR REPLACE VIEW datahub_schema.hdb_custom_types AS
SELECT hdb_custom_types.custom_types
FROM hdb_catalog.hdb_custom_types;
-- datahub_schema.hdb_function source
CREATE OR REPLACE VIEW datahub_schema.hdb_function AS
...
CREATE OR REPLACE VIEW datahub_schema.hdb_permission AS
...
CREATE OR REPLACE VIEW datahub_schema.hdb_relationship AS
...
CREATE OR REPLACE VIEW datahub_schema.hdb_remote_relationship AS
...
CREATE OR REPLACE VIEW datahub_schema.hdb_scheduled_event_invocation_logs AS
...
CREATE OR REPLACE VIEW datahub_schema.hdb_scheduled_events AS
...
CREATE OR REPLACE VIEW datahub_schema.hdb_table AS
SELECT hdb_table.table_schema,
   ...
   (
      SELECT concat('"', hdb_table.table_schema, '"') AS concat
   ) AS schema_reference
FROM hdb_catalog.hdb_table;
-- datahub_schema.key_column_usage source
CREATE OR REPLACE VIEW datahub_schema.key_column_usage AS
SELECT columns.table_catalog,
   columns.table_schema,
   columns.table_name,
   columns.column_name,
...
CREATE OR REPLACE VIEW datahub_schema.referential_constraints AS
...
WHERE referential_constraints.unique_constraint_schema::name <> ALL (
      ARRAY ['pg_catalog'::name, 'information_schema'::name]
   );
-- datahub_schema.remote_schemas source
CREATE OR REPLACE VIEW datahub_schema.remote_schemas AS
...
CREATE OR REPLACE VIEW datahub_schema.role_column_grant AS
SELECT columns.table_catalog,
   columns.table_schema,
   columns.table_name,
   columns.column_name,
...
CREATE OR REPLACE VIEW datahub_schema."routines" AS
SELECT routines.specific_catalog,
   ...
   (
      SELECT concat('"', routines.routine_schema, '"') AS concat
   ) AS routine_schema_reference
FROM information_schema.routines
WHERE routines.specific_schema::name <> ALL (
      ARRAY ['pg_catalog'::name, 'information_schema'::name]
   );
-- datahub_schema.schemata source
CREATE OR REPLACE VIEW datahub_schema.schemata AS
SELECT schemata.catalog_name,
   ...
   schemata.sql_path,
   (
      SELECT concat('"', schemata.schema_name, '"') AS concat
   ) AS schema_reference
FROM information_schema.schemata;
-- datahub_schema."sequences" source
CREATE OR REPLACE VIEW datahub_schema."sequences" AS
...
WHERE sequences.sequence_schema::name <> ALL (
      ARRAY ['pg_catalog'::name, 'information_schema'::name]
   );
-- datahub_schema.table_constraints source
CREATE OR REPLACE VIEW datahub_schema.table_constraints AS
SELECT table_constraints.constraint_catalog,
   ...
   (
      SELECT concat('"', table_constraints.table_schema, '"') AS concat
   ) AS schema_reference
FROM information_schema.table_constraints
WHERE table_constraints.constraint_schema::name <> ALL (
      ARRAY ['pg_catalog'::name, 'information_schema'::name]
   );
-- datahub_schema.table_privileges source
CREATE OR REPLACE VIEW datahub_schema.table_privileges AS
SELECT table_privileges.grantor,
   ...
   (
      SELECT concat('"', table_privileges.table_schema, '"') AS concat
   ) AS schema_reference
FROM information_schema.table_privileges
WHERE table_privileges.table_schema::name <> ALL (
      ARRAY ['pg_catalog'::name, 'information_schema'::name]
   );
-- datahub_schema."tables" source
CREATE OR REPLACE VIEW datahub_schema."tables" AS
SELECT tables.table_catalog,
   ...
   (
      SELECT concat('"', tables.table_schema, '"') AS concat
   ) AS schema_reference
FROM information_schema.tables
WHERE tables.table_schema::name <> ALL (
      ARRAY ['pg_catalog'::name, 'information_schema'::name]
   );
-- datahub_schema.triggered_update_column source
CREATE OR REPLACE VIEW datahub_schema.triggered_update_column AS
SELECT triggers.trigger_catalog,
   ...
   (
      SELECT concat('"', triggers.trigger_schema, '"') AS concat
   ) AS schema_reference
FROM information_schema.triggers
WHERE triggers.trigger_schema::name <> ALL (
      ARRAY ['pg_catalog'::name, 'information_schema'::name]
   );
-- datahub_schema.triggers source
CREATE OR REPLACE VIEW datahub_schema.triggers AS
SELECT triggers.trigger_catalog,
   triggers.trigger_schema,
   triggers.trigger_name,
   triggers.event_manipulation,
...
CREATE OR REPLACE VIEW datahub_schema.view_column_usage AS
SELECT view_column_usage.view_catalog,
   ...
   (
      SELECT concat('"', view_column_usage.view_schema, '"') AS concat
   ) AS view_schema_reference,
   (
      SELECT concat('"', view_column_usage.table_schema, '"') AS concat
   ) AS table_schema_reference
FROM information_schema.view_column_usage
WHERE view_column_usage.view_schema::name <> ALL (
      ARRAY ['pg_catalog'::name, 'information_schema'::name]
   );
-- datahub_schema.view_routine_usage source
CREATE OR REPLACE VIEW datahub_schema.view_routine_usage AS
SELECT view_routine_usage.table_catalog,
   ...
   (
      SELECT concat('"', view_routine_usage.table_schema, '"') AS concat
   ) AS table_schema_reference
FROM information_schema.view_routine_usage
WHERE view_routine_usage.table_schema::name <> ALL (
      ARRAY ['pg_catalog'::name, 'information_schema'::name]
   );
-- datahub_schema.view_table_usage source
CREATE OR REPLACE VIEW datahub_schema.view_table_usage AS
SELECT view_table_usage.view_catalog,
   ...
   (
      SELECT concat('"', view_table_usage.view_schema, '"') AS concat
   ) AS view_schema_reference,
   (
      SELECT concat('"', view_table_usage.table_schema, '"') AS concat
   ) AS table_schema_reference
FROM information_schema.view_table_usage
WHERE view_table_usage.table_schema::name <> ALL (
      ARRAY ['pg_catalog'::name, 'information_schema'::name]
   );
-- datahub_schema."views" source
CREATE OR REPLACE VIEW datahub_schema."views" AS
SELECT views.table_catalog,
   ...
   (
      SELECT concat('"', views.table_schema, '"') AS concat
   ) AS schema_reference
FROM information_schema.views
WHERE views.table_schema::name <> ALL (
      ARRAY ['pg_catalog'::name, 'information_schema'::name]
   );